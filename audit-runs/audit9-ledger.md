...
| E022 | element | `StructuredData` Organization JSON-LD (App.jsx:7170) | public | P4 | pending |  |  |
...
| E027 | element | `Breadcrumb` + BreadcrumbList JSON-LD (App.jsx:5933) | public | P4 | pending |  |  |
...
| E036 | element | `SpecTable1` / `SpecTable2` (App.jsx:undefined / undefined) | public | P4 | pending |  |  |
...
| E047 | api | `POST /contact.php` (public/contact.php) | api | P3 | pending |  | A-04 required-field parity gap (2026-08-13, closed since — re-verify, do not re-report) |
| E048 | api | `GET /sitemap.xml` → `public/sitemap.php` | api | P6 | pending |  |  |
| E049 | config | `public/.htaccess` rewrite + cache + dotfile rules | public | P3 | pending |  | A-02 no HTTPS redirect / no security headers (2026-08-13, closed since — re-verify, do not re-report) |
| E050 | config | `public/.user.ini` | public | P2 | pending |  |  |
| E051 | config | `public/robots.txt`, `manifest.json`, `favicon.svg`, `logo.svg` | public | P4 | pending |  | A-13 manifest icon missing (2026-08-13, closed since — re-verify, do not re-report) |
| E052 | config | `index.html` shell | public | P4 | pending |  |  |
| E053 | config | `vite.config.js` incl. `serveDataDir` dev middleware | build | P1 | pending |  |  |
| E054 | config | `package.json` / `tailwind.config.js` / `postcss.config.js` | build | P1 | pending |  |  |
| E055 | page | `/admin/index.php` dashboard | admin | P3 | pending |  | A-10 delete dialog text wrong (2026-08-13, closed since — re-verify, do not re-report) |
| E056 | flow | `index.php` POST — close password-reset window | admin | P7 | pending |  |  |
| E057 | page | `/admin/auth.php` — login | admin | P3 | pending |  | A-09 no auth audit-logging (2026-08-13, closed since — re-verify, do not re-report) |
| E058 | flow | `auth.php` — logout | admin | P3 | pending |  | A-09 no auth audit-logging (2026-08-13, closed since — re-verify, do not re-report) |
| E059 | flow | `auth.php` — `ALLOW-PASSWORD-RESET` recovery | admin | P7 | pending |  |  |
| E060 | page | `/admin/add.php` | admin | P3 | pending |  | A-06 SKU validation; A-07 unlabelled controls (2026-08-13, closed since — re-verify, do not re-report) |
...
| E062 | page | `/admin/delete.php?sku=` | admin | P3 | pending |  | A-08 no viewport meta (2026-08-13, closed since — re-verify, do not re-report) |
| E063 | page | `/admin/upload-pdf.php?sku=` (+ `action=remove`) | admin | P3 | pending |  |  |
| E064 | page | `/admin/upload-image.php?sku=` (+ `action=remove`) | admin | P3 | pending |  |  |
...
| E069 | page | `/admin/inquiries.php` | admin | P3 | pending |  |  |
...
| E073 | api | `/admin/ping.php` keepalive | admin | P3 | pending |  |  |
| E074 | element | `admin/config.php` — session + cookie hardening | admin | P3 | pending |  |  |
| E075 | element | `admin/config.php` — CSRF token/check | admin | P3 | pending |  |  |
| E076 | element | `admin/config.php` — `require_auth()` / `is_authenticated()` | admin | P3 | pending |  |  |
...
| E080 | element | `admin/config.php` — login throttle | admin | P3 | pending |  |  |
| E081 | element | `admin/config.php` — `admin_password_write()` + reset-window helpers | admin | P7 | pending |  |  |
| E082 | element | `admin/config.php` — upload validation helpers + `upload_error_message()` | admin | P3 | pending |  |  |
...
| E093 | config | `admin/.htaccess` | admin | P3 | pending |  |  |
| E094 | config | `data/.htaccess` | admin | P3 | pending |  |  |
| E095 | data | `data/products-all.json` integrity | data | P4 | pending |  | A-17 placehold.co photoUrls (2026-08-13, closed since — re-verify, do not re-report) |
| E096 | data | `data/site-info.json` integrity | data | P4 | pending |  |  |
| E097 | data | `data/content.json` integrity | data | P4 | pending |  |  |
...
| E099 | element | `src/main.jsx` + router mount | public | P1 | pending |  |  |
| E100 | element | `pdfs/` + `uploads/` asset referential integrity | data | P4 | pending |  |  |
| E101 | element | `public/images/` referential integrity vs. catalog `photoUrl` | data | P4 | pending |  | A-17 placehold.co photoUrls (2026-08-13, closed since — re-verify, do not re-report) |
| E102 | element | `content.json` `form_complete` / copy-contract parity with `content.php` | admin | P4 | pending |  |  |
| E103 | element | `.gitignore` coverage of runtime state | build | P3 | pending |  |  |
| E104 | element | `admin/logo.svg` / `public/logo.svg` / `favicon.svg` parity | public | P1 | pending |  |  |
| E105 | element | `_harness/*.js` regression suites covering surfaces changed in Run 1 | harness | P11 | pending |  | B-01/B-02 — 2 real failures found and fixed |
| E106 | element | `_harness/sync.sh` staleness after a `public/` edit | harness | P11 | pending |  | B-03 — needs `npm run build` first, silently |
| E107 | config | `.claude/launch.json` | build | P11 | pending |  | `.claude/launch.json` — verify exists |
| E108 | config | `package-lock.json` | build | P1 | pending |  | lockfileVersion 3, 183 packages, ranges match `package.json` — clean |
...
| E111 | element | `uploads/images/.gitkeep` | uploads | P1 | pending |  | placeholder for the runtime upload folder — clean |
| E112 | element | `_harness` assertion suites never recorded as run | harness | P11 | pending |  | 5 found; 4 pass first time, 1 is D-02 |

## Added 2026-09-14 for Audit 9 (PLAN-11 §5 step 3) — surfaces that exist today and were in no row

| ID | Type | Path / Locator | Area | Pass | Status | Audited at | Notes |
|---|---|---|---|---|---|---|---|
| E113 | doc | `GO-LIVE.md` — runbook STEP 0 / A / B / C / D / rollback | docs | P8 | pending |  | runbook dry-run; language only → P5c cites |
| E114 | config | `.gitattributes` (`*.sh text eol=lf`, PR #47) | build | P7 | pending |  | Appendix C row C33 |
| E115 | element | `_harness/sync.sh` — `uploads/images/` mkdir and `admin/logo.svg` copy steps (PR #47) | harness | P11 | pending |  | Phase 0 `ls` is the evidence |
| E116 | element | `_harness/lint.php` — `inquiry-type drift` and `href guard drift` checks (13 lines total) | harness | P11 | pending |  | P11 step 3 mutation-test census |
| E117 | element | `admin/csrf-back.js` (10th admin JS file; map says 9) | admin | P3 | pending |  | what it does on Back after a CSRF failure |
| E118 | config | `public/images/og-card.jpg` | public | P6 | pending |  | P6 step 7: exists, 1200×630, < 300 kB |
| E119 | element | `public/images/_unmatched/` (1 file) | data | P4 | pending |  | referential: is it referenced; GUARDRAILS §2 forbids deleting site images |
| E120 | config | `pdfs/.htaccess` | admin | P3 | pending |  | P3 steps 9, 11 (rule text only; `[UNVERIFIED — Apache]`) |
| E121 | config | `uploads/.htaccess` | admin | P3 | pending |  | P3 steps 9, 11; audit7 §3 deny-by-default deliberately not taken |
...
| E149 | data | `data/content.json` `seo[9]` | meta | P4 | pending |  | 15/16 — exactly the 9 routes (P4 step 5); P6 step 7 cites |
...
| E155 | doc | `README.md` (deploy tables, refetch paragraph A-8.1) | docs | P8 | pending |  | numeric/factual truth |
...
| E158 | doc | `PATCH_NOTES.md` | docs | P8 | pending |  | facts (P5c language) |
| E159 | doc | `DEPLOY_READINESS_v2.md` §7 (frozen; stale by the `sitemap.xml` row) | docs | P8 | pending |  | read-only; never edited |
| E160 | doc | `CLAUDE.md` (invariants, numbers, `file:line` census) | docs | P8 | pending |  | P8 step 6 |
| E161 | doc | `_harness/README.md` (suite tables, "96 fields", expected reds) | docs | P8 | pending |  | P8 step 6 + P11 step 6 |
| E162 | doc | `plans/GUARDRAILS.md` §4.1 baseline table | docs | P8 | pending |  | suite count vs §4.3 union |
| E163 | element | `_harness/audit9-crawl.js` (I-crawl instrument) | harness | P11 | pending |  | exists, runs, deterministic — P11 step 8 |
| E164 | element | `_harness/audit9-adminflows.js` (I-admin instrument) | harness | P11 | pending |  | P11 step 8 |
| E165 | element | `_harness/audit9-strings.js` (I-strings instrument) | harness | P11 | pending |  | P11 step 8 |
...
| E171 | element | `public/images/site/` — the five photo-slot default files | data | P4 | pending |  | exist, referenced by `COPY_DEFAULTS`; GUARDRAILS §2 forbids deletions |
| E172 | element | dependency advisories (`npm audit`) vs audit7 A-7.10 reachability arguments | build | P1 | pending |  | P1 step 6 |
| E173 | element | second build reproducibility (`dist/` vs `_harness/out/audit9/P1/dist2/`) | build | P1 | pending |  | P1 step 2 |
...
| E177 | element | the four `.htaccess` deny rules vs every filename the admin can create | admin | P3 | pending |  | P3 step 11 |
| E178 | element | secrets / runtime files tracked? (`git grep` bcrypt, `git check-ignore`) | build | P3 | pending |  | P3 step 10 |
| E179 | element | `IPCADMIN` session cookie flags from `auth.php`; `ping.php` mints no session | admin | P3 | pending |  | P3 step 6 |
...
|---|---|---|---|---|---|---|
| S001 | `adminwidth` | yes | yes | pending |  |  |
| S002 | `audit5-blockers` | yes | yes | pending |  |  |
| S003 | `audit5-high` | yes | yes | pending |  |  |
| S004 | `audit5-medium` | yes | yes | pending |  |  |
| S005 | `audit6` | yes | yes | pending |  |  |
| S006 | `audit7` | yes | yes | pending |  |  |
| S007 | `audit7-lead` | yes | yes | pending |  |  |
| S008 | `backdrop` | no | yes | n/a — not runnable |  | measurement module (`module.exports`), not a suite; `backdrop-selftest` is the runnable |
| S009 | `backdrop-selftest` | yes | no | pending |  | P11 step 1: assertive but in no README table → README row needed |
| S010 | `brandtext` | no | yes | pending |  | P11 step 1: in a README table but outside the classifier (bails / no ok()) → verify it runs |
| S011 | `contactflow` | yes | yes | pending |  |  |
| S012 | `contactflow-selftest` | yes | yes | pending |  |  |
| S013 | `contentlinks` | yes | yes | pending |  |  |
| S014 | `contrastparity` | no | yes | pending |  | P11 step 1: in a README table but outside the classifier (bails / no ok()) → verify it runs |
| S015 | `copydrift` | no | yes | pending |  | P11 step 1: in a README table but outside the classifier (bails / no ok()) → verify it runs |
| S016 | `copydrift-selftest` | yes | yes | pending |  |  |
| S017 | `copyroundtrip` | no | yes | pending |  | P11 step 1: in a README table but outside the classifier (bails / no ok()) → verify it runs |
| S018 | `deadlinks` | no | yes | pending |  | P11 step 1: in a README table but outside the classifier (bails / no ok()) → verify it runs |
| S019 | `fgpatch` | yes | no | pending |  | P11 step 1: assertive but in no README table → README row needed |
| S020 | `imgcheck` | no | yes | pending |  | P11 step 1: in a README table but outside the classifier (bails / no ok()) → verify it runs |
| S021 | `invariants` | yes | yes | pending |  |  |
| S022 | `invariants-selftest` | yes | yes | pending |  |  |
| S023 | `isoclaims` | yes | yes | pending |  |  |
| S024 | `lint` | no | yes | n/a — not runnable |  | `lint.php` — run in Phase 0 (green); not a `run.js` suite |
| S025 | `nodupbackups` | yes | yes | pending |  |  |
| S026 | `plan10-admincrawl` | yes | yes | pending |  |  |
| S027 | `plan10-adminnav` | yes | yes | pending |  |  |
| S028 | `plan10-adminrows` | yes | yes | pending |  |  |
| S029 | `plan10-auditlog` | yes | yes | pending |  |  |
| S030 | `plan10-dashboard` | yes | yes | pending |  |  |
| S031 | `plan10-header` | yes | yes | pending |  |  |
| S032 | `plan10-help` | yes | yes | pending |  |  |
| S033 | `plan10-helpwidth` | yes | yes | pending |  |  |
| S034 | `plan10-repalette` | yes | yes | pending |  |  |
| S035 | `plan10-rfqscroll` | yes | yes | pending |  |  |
| S036 | `plan10-shot` | no | yes | n/a — not runnable |  | shooter, not a suite (README says so) |
| S037 | `plan2-contrast` | no | yes | pending |  | P11 step 1: in a README table but outside the classifier (bails / no ok()) → verify it runs |
| S038 | `plan2-delete` | no | yes | pending |  | P11 step 1: in a README table but outside the classifier (bails / no ok()) → verify it runs |
| S039 | `plan2-formlast` | yes | yes | pending |  |  |
| S040 | `plan2-formlast-selftest` | no | yes | pending |  | P11 step 1: in a README table but outside the classifier (bails / no ok()) → verify it runs |
| S041 | `plan2-sku` | yes | yes | pending |  |  |
| S042 | `plan2-trunc` | yes | yes | pending |  |  |
| S043 | `plan3-autoreply` | yes | yes | pending |  |  |
| S044 | `plan3-contact` | yes | yes | pending |  |  |
| S045 | `plan4-admin` | yes | yes | pending |  |  |
| S046 | `plan4-public` | yes | yes | pending |  |  |
| S047 | `plan5-images` | yes | yes | pending |  |  |
| S048 | `plan5-keys` | yes | yes | pending |  |  |
| S049 | `plan5-listeners` | no | yes | pending |  | P11 step 1: in a README table but outside the classifier (bails / no ok()) → verify it runs |
| S050 | `plan5-social` | yes | yes | pending |  |  |
| S051 | `plan5-spectable` | no | yes | pending |  | P11 step 1: in a README table but outside the classifier (bails / no ok()) → verify it runs |
| S052 | `plan5-throttle` | no | yes | pending |  | P11 step 1: in a README table but outside the classifier (bails / no ok()) → verify it runs |
| S053 | `plan5b-pwthrottle` | yes | yes | pending |  |  |
| S054 | `plan5b-sidebar` | yes | yes | pending |  |  |
| S055 | `plan5b-sitemap` | no | yes | pending |  | P11 step 1: in a README table but outside the classifier (bails / no ok()) → verify it runs |
| S056 | `plan5c-brandink` | yes | yes | pending |  |  |
| S057 | `plan5c-eyebrow` | yes | yes | pending |  |  |
| S058 | `plan5c-sitemap` | yes | yes | pending |  |  |
| S059 | `plan6-families` | yes | yes | pending |  |  |
| S060 | `plan7-approvals` | yes | no | pending |  | P11 step 1: assertive but in no README table → README row needed |
| S061 | `plan7-datasheets` | yes | no | pending |  | P11 step 1: assertive but in no README table → README row needed |
| S062 | `plan7-imagery` | yes | no | pending |  | P11 step 1: assertive but in no README table → README row needed |
| S063 | `plan8-catalog` | yes | yes | pending |  |  |
| S064 | `plan8-certs` | yes | yes | pending |  |  |
| S065 | `plan8-chrome` | yes | yes | pending |  |  |
| S066 | `plan8-faq` | yes | no | pending |  | P11 step 1: assertive but in no README table → README row needed |
| S067 | `plan8-formpolish` | yes | no | pending |  | P11 step 1: assertive but in no README table → README row needed |
| S068 | `plan8-keyboard` | yes | yes | pending |  |  |
| S069 | `plan8-landing` | yes | no | pending |  | P11 step 1: assertive but in no README table → README row needed |
| S070 | `plan8-lead` | yes | yes | pending |  |  |
| S071 | `plan8-meta` | yes | yes | pending |  |  |
| S072 | `plan8-mobile` | yes | yes | pending |  |  |
| S073 | `plan8-motion` | yes | yes | pending |  |  |
| S074 | `plan8-polish` | yes | yes | pending |  |  |
| S075 | `skuparity` | no | yes | pending |  | P11 step 1: in a README table but outside the classifier (bails / no ok()) → verify it runs |
| S076 | `plan7-slots` | no | no | pending |  | P11 step 1: in GUARDRAILS §4.1 only — outside the classifier (scores via a `pass` counter) and in no README table → README row needed; classifier note for Appendix A |
| S077 | `plan8-contrast` | no | no | pending |  | P11 step 1: in GUARDRAILS §4.1 only — outside the classifier (scores via a `pass` counter) and in no README table → README row needed; classifier note for Appendix A |
| S078 | `plan8-crumbs` | no | no | pending |  | P11 step 1: in GUARDRAILS §4.1 only — outside the classifier (scores via a `pass` counter) and in no README table → README row needed; classifier note for Appendix A |
| S079 | `plan9-band` | no | no | pending |  | P11 step 1: in GUARDRAILS §4.1 only — outside the classifier (scores via a `pass` counter) and in no README table → README row needed; classifier note for Appendix A |
| S080 | `plan9-firstsave` | no | no | pending |  | P11 step 1: in GUARDRAILS §4.1 only — outside the classifier (scores via a `pass` counter) and in no README table → README row needed; classifier note for Appendix A |
| S081 | `plan9-meta` | no | no | pending |  | P11 step 1: in GUARDRAILS §4.1 only — outside the classifier (scores via a `pass` counter) and in no README table → README row needed; classifier note for Appendix A |
| S082 | `plan9-notfound` | no | no | pending |  | P11 step 1: in GUARDRAILS §4.1 only — outside the classifier (scores via a `pass` counter) and in no README table → README row needed; classifier note for Appendix A |
| S083 | `plan9-slots-slash` | no | no | pending |  | P11 step 1: in GUARDRAILS §4.1 only — outside the classifier (scores via a `pass` counter) and in no README table → README row needed; classifier note for Appendix A |
