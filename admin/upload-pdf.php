...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (($_POST['action'] ?? '') === 'remove' && !empty($currentPdf)) {
        ...
        if (save_products($products)) {
            ...
            // outside the upload folder, even if pdfUrl was tampered with. And
    ...
    elseif (!isset($_FILES['pdf_file']) || $_FILES['pdf_file']['error'] !== UPLOAD_ERR_OK) {
        ...
        if ($ext !== 'pdf' || $mimeType !== 'application/pdf') {
            ...
            //    reusing the existing filename. This keeps family/compound
            //    datasheets (e.g. IP17TW-IP18SW-IP19LW.pdf) and custom names
            //    (e.g. IP52EC-molded-cap.pdf) stable — the new file overwrites
            //    the old one, the pdfUrl is unchanged, and no orphan is left.
            //  - If there is no PDF yet, derive a fresh name from the SKU.
            $existingName = $currentPdf !== '' ? basename($currentPdf) : '';
            if ($existingName !== '' && preg_match('/^[A-Za-z0-9._-]+\.pdf$/', $existingName)) {
  ...
  <div class="card">
    ...
    <form method="POST" enctype="multipart/form-data">
      ...
      <?php
        ...
        // exists, otherwise derive the name from the SKU.
        $existingName2 = $currentPdf !== '' ? basename($currentPdf) : '';
        if ($existingName2 !== '' && preg_match('/^[A-Za-z0-9._-]+\.pdf$/', $existingName2)) {
            $pdfName2 = $existingName2;
        } else {
