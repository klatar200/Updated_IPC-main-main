    ...
    if ($action === 'preview') {
        ...
        if (!isset($_FILES['json_file']) || $_FILES['json_file']['error'] !== UPLOAD_ERR_OK) {
            ...
            if (!is_array($data)) {
                ...
                $incoming  = import_extract_rows($data);
        ...
        if (!is_array($data)) { $errors[] = 'Invalid JSON data.'; }
            ...
            foreach ($incoming as $p) {
                ...
                if ($idx === -1) {
                    ...
                    // FIELD-LEVEL MERGE (F1): only overwrite fields the imported
                    // row actually included. Any field omitted from the import
                    // keeps its current value, so a partial import (e.g. fixing
                    // just a name) can no longer wipe description, spec tables,
                    // badges, pdfLabel, additionalPdfs, etc. id/sku always sync.
                    $merged = $existing[$idx];
                    foreach ($clean as $key => $val) {
                        if ($key === 'id' || $key === 'sku' || array_key_exists($key, $p)) {
                            $merged[$key] = $val;
                        }
                    }
                    $existing[$idx] = $merged;
...
<body>
<header><a class="logo" href="index.php" style="display:inline-flex;align-items:center;gap:10px;"><img src="/logo.svg" alt="IPC" style="width:32px;height:32px;border-radius:6px;display:block;"><span>Admin</span></a></header>
<main>
  ...
  <?php if ($preview): ?>
    <div class="card">
      ...
      <?php endif; ?>
      <p style="font-size:13px;color:#6b7280">Existing products with matching SKUs are <strong>merged field-by-field</strong> — any field you leave out of the import keeps its current value. New SKUs are <strong>appended</strong>. Nothing is deleted. Invalid rows are skipped.</p>
      <form method="POST">
