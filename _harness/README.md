...
| `isoclaims.js` | **EXPECTED RED — 2/4, and that is the finding, not a break.** Every ISO 9001 revision string in the three live JSON files, grouped, with the admin screen each is edited on. FAILS while a *withdrawn* revision is present: `data/content.json` has three `ISO 9001:2008` (withdrawn Sept 2018) and `data/products-all.json` three `ISO9001:2000` (superseded Nov 2008) on VALUE-ADDED — and three of the six are under Products → Edit, **not** Page Content, which is where every prior owner-action line pointed. It has a real pass state and goes green when the six are resolved. It deliberately does **not** rewrite a claim: writing `:2015` because it is current would invent a certification for a supplier to aerospace, medical and automotive. Also asserts `site-info.json` still holds the bare unversioned form — that field reaches every page. Needs no server. (audit-runs/audit8.md A-8.5) |
...
| `audit7.js` | **A-7.8 and A-7.9** of `audit-runs/audit7.md`. 30 checks. A-7.8 writes six malformed spec/PDF shapes into the mirror catalog one at a time and requires the product page to render rather than throw into the ErrorBoundary — each arm scored against an untouched neighbouring product on the same catalog load, because "the page is broken" and "the mirror is broken" are otherwise the same observation. A-7.9 routes `/data/*.json` to a handler that accepts and never responds, then asks the browser what became of each request after 15 s; the catalog fetch is the control, since it already carries the 12 s abort and must be seen to fire. Mutates the mirror only, restores from `pristine/` in a `finally`, and asserts the restore byte-for-byte before reporting |
| `audit7-lead.js` | **A-7.1 through A-7.6** — the lead path, the session store, and the two gaps in audit 6's own fixes. 23 checks, no browser: HTTP, filesystem and PHP-level, which is why it is a second file rather than a mode bolted onto a browser suite. Every arm carries the control that the fix could plausibly have broken — a valid submission still logging exactly once (A-7.1), the `fetch()` path still receiving JSON with **and** without an Accept header (A-7.2), `auth.php` still minting a session (A-7.3), a healthy log write leaving no marker (A-7.4). The A-7.4 failure is injected by replacing `inquiries.jsonl` with a **directory**, because `chmod` does not stop root in this container. Clears the per-IP limiter between arms — every request comes from 127.0.0.1, so a suite that just fires submissions hits its own 429 and measures the limiter instead |
